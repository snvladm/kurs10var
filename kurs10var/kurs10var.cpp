...
bool compaire(const student& stud1, const student& stud2) {
        int firstMarks3 = 0;
        int secondMarks3 = 0;
        for (auto less = stud1.marks[0].begin(); less != stud1.marks[0].end(); less++){
                if (less->mark == 3) firstMarks3++;
}
        for (auto less = stud1.marks[1].begin(); less != stud1.marks[1].end(); less++){
                if (less->mark == 3) firstMarks3++;
}
        for (auto less = stud2.marks[0].begin(); less != stud2.marks[0].end(); less++){
                if (less->mark == 3) secondMarks3++;
}

        for (auto less = stud2.marks[1].begin(); less != stud2.marks[1].end(); less++){
                if (less->mark == 3) secondMarks3++;
}
        return float(firstMarks3) / (stud1.marks[0].size() + stud1.marks[1].size()) < float(secondMarks3) / (stud2.marks[0].size() + stud2.marks[1].size());
};
